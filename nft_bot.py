...
# ── Стикерпаки ────────────────────────────────────────────────────────────────
CB_STICKERPACK_PROFILE = "spp:"   # создать стикерпак из профиля
CB_STICKERPACK_MY      = "spm"    # показать мои стикерпаки
CB_ADD_TO_PERSONAL     = "sadd:"  # добавить стикер в личный пак
...
# ── Стикерпак: хранилище ──────────────────────────────────────────────────────
# uid -> {"pack_name": str, "created_at": float, "count": int}
_profile_pack:  dict[int, dict] = {}
# uid -> {"pack_name": str, "created_at": float, "slugs": list[str]}
_personal_pack: dict[int, dict] = {}
# Кулдаун обновления пака: uid -> last_update (monotonic)
_pack_update_cd: dict[int, float] = {}
PACK_UPDATE_COOLDOWN = 86400.0   # 1 раз в сутки
...
def make_keyboard_video(slug: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="➕ Добавить в личный стикерпак",
...
async def cmd_start(message: Message) -> None:
    ...
    buttons.append([InlineKeyboardButton(
        text="🎴 Мои стикерпаки",
        callback_data=CB_STICKERPACK_MY,
    )])
    buttons.append([InlineKeyboardButton(
...
def _pack_short_name(uid: int, kind: str, bot_username: str) -> str:
    """
    Генерирует имя стикерпака по правилам Telegram:
    только латиница, цифры, подчёркивание, заканчивается на _by_<botname>.
    kind: "profile" | "personal"
    """
    return f"nft_{kind}_{uid}_by_{bot_username}"


def _pack_title(username: Optional[str], kind: str) -> str:
    uname = f"@{username}" if username else "пользователя"
    if kind == "profile":
        return f"Профиль {uname} | @{BOT_USERNAME}"
    return f"Личный {uname} | @{BOT_USERNAME}"


async def _upload_tgs_sticker(tgs_data: bytes, uid: int) -> Optional[str]:
    """Загружает TGS на серверы Telegram, возвращает file_id."""
    try:
        from aiogram.types import InputSticker, BufferedInputFile as BIF
        f = BIF(tgs_data, filename="sticker.tgs")
        ...
        logger.error("_upload_tgs_sticker: %s", e)
        return None


async def _create_sticker_set(uid: int, pack_name: str, pack_title: str,
                               file_id: str) -> bool:
    """Создаёт новый стикерпак с первым стикером."""
    try:
        from aiogram.types import InputSticker
        sticker = InputSticker(sticker=file_id, emoji_list=["🎁"], format="animated")
        ...
        logger.error("_create_sticker_set: %s", e)
        return False


async def _add_sticker_to_set(uid: int, pack_name: str, file_id: str) -> bool:
    """Добавляет стикер в существующий пак."""
    try:
        from aiogram.types import InputSticker
        sticker = InputSticker(sticker=file_id, emoji_list=["🎁"], format="animated")
        ...
        logger.error("_add_sticker_to_set: %s", e)
        return False


async def _pack_exists(pack_name: str) -> bool:
    """Проверяет существует ли стикерпак."""
    try:
...
# ── Создание стикерпака из профиля (getUserGifts) ─────────────────────────────

async def build_profile_pack(message: Message, user: object) -> None:
    """
    Получает NFT подарки пользователя через getUserGifts,
    создаёт или обновляет стикерпак "Профиль @username".
    """
    uid      = user.id
    username = user.username or str(uid)
    pack_name  = _pack_short_name(uid, "profile", BOT_USERNAME)
    pack_title = _pack_title(user.username, "profile")

    # Кулдаун: обновлять не чаще 1 раза в сутки
    now = time.monotonic()
    existing = _profile_pack.get(uid)
    if existing:
        last_upd = _pack_update_cd.get(uid, 0.0)
        if now - last_upd < PACK_UPDATE_COOLDOWN:
            remaining = int(PACK_UPDATE_COOLDOWN - (now - last_upd))
            h, m = remaining // 3600, (remaining % 3600) // 60
            await message.answer(
                f"⏳ <b>Стикерпак уже создан.</b>\n\n"
                f"Следующее обновление возможно через <code>{h}ч {m}мин</code>.\n\n"
    ...
    if not nft_gifts:
        await safe_delete(wm)
        await message.answer(
            "😔 <b>NFT подарков не найдено.</b>\n\n"
            "Убедитесь, что у вас есть уникальные (NFT) подарки в профиле\n"
            "и список подарков открыт для всех.",
            parse_mode=ParseMode.HTML,
        )
        return

    total = min(len(nft_gifts), 100)
    nft_gifts = nft_gifts[:total]

    await safe_delete(wm)
    progress_msg = await message.answer(
        f"⚙️ <b>Создаю стикерпак…</b>\n\n"
        f"{_progress_bar(0, total)} <code>0 / {total}</code>\n\n"
        f"<i>Это может занять до {total * 2} секунд</i>",
        parse_mode=ParseMode.HTML,
    )

    pack_already_exists = await _pack_exists(pack_name)
    added = 0
    errors = 0

    for i, file_id in enumerate(nft_gifts):
        try:
            ...
            logger.warning("build_profile_pack sticker %d: %s", i, e)
            errors += 1

        # Обновляем прогресс каждые 5 штук
        if (i + 1) % 5 == 0 or i == total - 1:
            try:
                await progress_msg.edit_text(
                    f"⚙️ <b>Создаю стикерпак…</b>\n\n"
                    f"{_progress_bar(i + 1, total)} <code>{i + 1} / {total}</code>\n\n"
                    f"<i>Добавлено: {added} | Ошибок: {errors}</i>",
                    parse_mode=ParseMode.HTML,
                )
            except Exception:
                pass

        await asyncio.sleep(0.05)  # небольшая пауза чтобы не флудить API

    # Сохраняем метаданные
    _profile_pack[uid] = {"pack_name": pack_name, "count": added}
    _pack_update_cd[uid] = time.monotonic()

    await safe_delete(progress_msg)

    if added == 0:
        await message.answer(
            "❌ <b>Не удалось добавить ни одного стикера.</b>\n"
            "Попробуй позже.",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    await message.answer(
        f"✅ <b>Стикерпак готов!</b>\n\n"
        f"🎁 Добавлено стикеров: <b>{added}</b>{f' (ошибок: {errors})' if errors else ''}\n\n"
        f"🔗 <a href='https://t.me/addstickers/{pack_name}'>Открыть и добавить стикерпак</a>",
        parse_mode=ParseMode.HTML,
        disable_web_page_preview=True,
    )
    user_log.info("✅ ПРОФИЛЬ ПАК | uid=%d | добавлено=%d", uid, added)


# ── Добавление одного стикера в личный пак ────────────────────────────────────

async def add_to_personal_pack(callback: object, uid: int, username: Optional[str],
                                slug: str) -> None:
    """
    Добавляет TGS стикер конкретного NFT в личный пак пользователя.
    Если пак не существует — создаёт.
    """
    pack_name  = _pack_short_name(uid, "personal", BOT_USERNAME)
    pack_title = _pack_title(username, "personal")

    # Проверяем не добавлен ли уже
    personal = _personal_pack.get(uid, {})
    slugs = personal.get("slugs", [])
    if slug.lower() in [s.lower() for s in slugs]:
        await callback.answer("✅ Этот стикер уже есть в вашем личном стикерпаке!", show_alert=False)
        return

    # Лимит 120 (ограничение Telegram для анимированных паков)
    if len(slugs) >= 120:
        await callback.answer(
            "⚠️ Ваш личный стикерпак заполнен (120/120).",
            show_alert=True,
        )
        return

    await callback.answer("⏳ Добавляю в стикерпак…", show_alert=False)

    # Загружаем TGS
    found, tgs_data, err = await fetch_nft_tgs(slug)
    if not found or not tgs_data:
        try:
            await callback.message.answer(
                f"❌ Не удалось загрузить стикер: {err or 'файл не найден'}",
                parse_mode=ParseMode.HTML,
            )
        except Exception:
            pass
    ...
    file_id = await _upload_tgs_sticker(tgs_data, uid)
    if not file_id:
        try:
            await callback.message.answer(
                "❌ Не удалось загрузить стикер на серверы Telegram.",
                parse_mode=ParseMode.HTML,
            )
        except Exception:
            pass
    ...
    pack_exists = _personal_pack.get(uid, {}).get("pack_name") and await _pack_exists(pack_name)

    if pack_exists or (await _pack_exists(pack_name)):
        ok = await _add_sticker_to_set(uid, pack_name, file_id)
    else:
        ok = await _create_sticker_set(uid, pack_name, pack_title, file_id)
    ...
    if not ok:
        try:
            await callback.message.answer(
                "❌ Не удалось добавить стикер в пак. Попробуй позже.",
                parse_mode=ParseMode.HTML,
            )
        except Exception:
            pass
    ...
    # Обновляем состояние
    if uid not in _personal_pack:
        _personal_pack[uid] = {"pack_name": pack_name, "slugs": []}
    _personal_pack[uid]["slugs"].append(slug.lower())

    count = len(_personal_pack[uid]["slugs"])
    try:
        await callback.message.answer(
            f"✅ <b>Стикер добавлен в личный пак!</b>\n\n"
            f"В паке: <b>{count}/120</b> стикеров\n"
            f"🔗 <a href='https://t.me/addstickers/{pack_name}'>Открыть стикерпак</a>",
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True,
        )
        ...
        pass
    user_log.info("✅ ЛИЧНЫЙ ПАК +1 | uid=%d | slug=%s | total=%d", uid, slug, count)


# ── Команда /mypacks — мои стикерпаки ────────────────────────────────────────

def _my_packs_text_and_markup(uid: int) -> tuple[str, InlineKeyboardMarkup]:
    profile  = _profile_pack.get(uid)
    personal = _personal_pack.get(uid)

    has_any = profile or personal
    text = (
        "🎴 <b>Мои стикерпаки</b>\n"
        "<code>━━━━━━━━━━━━━━━━━━━━</code>\n\n"
    )
    ...
    if not has_any:
        text += (
            ...
            "• Нажмите <b>«Создать из профиля»</b> — бот возьмёт все ваши NFT подарки\n"
            "• Или нажмите <b>«+ в пак»</b> под любым видео-превью, чтобы добавить отдельный стикер"
        )
    else:
        if profile:
            pname = profile["pack_name"]
            count = profile.get("count", "?")
            text += (
                "🟣 <b>Стикерпак профиля</b>\n"
                f"   <code>{count}</code> стикеров · все NFT из вашего профиля\n"
                f"   🔗 <a href='https://t.me/addstickers/{pname}'>Открыть / добавить</a>\n\n"
            )
        if personal:
            pname = personal["pack_name"]
            count = len(personal.get("slugs", []))
            text += (
                "🔵 <b>Личный стикерпак</b>\n"
                f"   <code>{count}/120</code> стикеров · выбранные вручную\n"
                f"   🔗 <a href='https://t.me/addstickers/{pname}'>Открыть / добавить</a>\n\n"
            )
    ...
    buttons = []
    buttons.append([InlineKeyboardButton(
        text="🟣 Создать стикерпак из профиля",
        callback_data=CB_STICKERPACK_PROFILE,
    )])
...
@dp.callback_query(F.data == CB_STICKERPACK_MY)
async def callback_my_packs(callback: CallbackQuery) -> None:
    """Показывает мои стикерпаки."""
    await callback.answer()
    uid = callback.from_user.id
    text, markup = _my_packs_text_and_markup(uid)
    await callback.message.answer(text, parse_mode=ParseMode.HTML,
                                   reply_markup=markup,
                                   disable_web_page_preview=True)


@dp.callback_query(F.data == CB_STICKERPACK_PROFILE)
async def callback_profile_pack(callback: CallbackQuery) -> None:
    """Создаёт/обновляет стикерпак из NFT-профиля пользователя."""
    await callback.answer()
    await build_profile_pack(callback.message, callback.from_user)


@dp.callback_query(F.data.startswith(CB_ADD_TO_PERSONAL))
async def callback_add_to_personal(callback: CallbackQuery) -> None:
    """Добавляет один стикер в личный пак. Кнопка НЕ исчезает."""
    uid      = callback.from_user.id
    ...
    await add_to_personal_pack(callback, uid, username, slug)
    # Намеренно НЕ удаляем кнопку — каждый может добавить в свой пак


# ══════════════════════════════════════════════════════════════════════════════
#  ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ОБРАБОТКИ СПАМ-РЕЗУЛЬТАТА
# ══════════════════════════════════════════════════════════════════════════════
